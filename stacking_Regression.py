...
# Meta model using linear regerssion model (3 base learner, 1 meta learner)
...
meta_data = np.zeros((len(base_learners), len(train_x))) # (3, 400)
meta_targets = np.zeros(len(train_x)) # output (400 output, actual data)

# Create the cross-validation folds
KF = KFold(n_splits = 5) # 5 time iteration
meta_index = 0 # starts from zero

for train_indices, test_indices in KF.split(train_x): # divide into 320(train_indices) - 80(test_indices)
  # Train each learner on the K-1 folds 
  # and create metadata for the Kth fold
  for i in range(len(base_learners)): #(0,1,2) - 3 base learners
    learner = base_learners[i]
    learner.fit(train_x[train_indices], train_y[train_indices])
    predictions = learner.predict(train_x[test_indices])
    meta_data[i][meta_index:meta_index + len(test_indices)] = predictions # capture all the predictions in meta data

  meta_targets[meta_index:meta_index + len(test_indices)] = train_y[test_indices] # actual data
  meta_index += len(test_indices)

# Transpose the metadata to be fed into the meta-learner
meta_data = meta_data.transpose() #(400,3)


# Create the metadata for the test set and evaluate the base learners
test_meta_data = np.zeros((len(base_learners), len(test_x))) #3,400
base_errors = []
base_r2 = []
